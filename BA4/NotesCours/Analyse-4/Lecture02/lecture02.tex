...
\parag{Example 2: Construction d'une fonction holomorphe}{
...
\parag{Définition: Exponentielle complexe}{
...
\parag{Théorème: Propriétés de l'exponentielle}{
...
\parag{Définition: Logarithme complexe}{
    ...
    \subparag{Cours de Prof. David Strütt}{
        ...
        Cependant, alors: 
        \[x + iy = e^{f\left(z\right)} = e^{u\left(x, y\right) + iv\left(x, y\right)} = e^{u\left(x, y\right)}\left(\cos\left(v\left(x, y\right)\right) + i\sin\left(x, y\right)\right)\]
        
        On remarque qu'on a obtenu la forme polaire d'un nombre complexe. Or, pour que deux nombres complexes soit égaux, il faut que leur argument soient égaux, et que leur argument sont égaux modulo $2\pi$, donc: 
        \[\begin{systemofequations} e^{u\left(x, y\right)} = \left\|x + yi\right\| \iff u\left(x, y\right) = \log\left(\left|z\right|\right) \\ \congruent{v\left(x, y\right)}{\arg\left(z\right)}{2\pi} \end{systemofequations}\]
...
\parag{Théorème: Propriétés du logarithme}{
    \begin{enumerate}[left=0pt]
