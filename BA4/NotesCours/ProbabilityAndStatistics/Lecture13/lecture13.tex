...
\parag{Definition: Expectation}{
    ...
    If $X, Y$ are discrete and the following sum converge for $\left|g\left(X, Y\right)\right|$, then the \important{expectation} of $g\left(X, Y\right)$ is: 
    \[\exval\left[g\left(X, Y\right)\right] = \sum_{x, y}^{} g\left(x, y\right)f_{X, Y}\left(x, y\right)\]
    
    If $X, Y$ are continuous and the following integral converge for $\left|g\left(X, Y\right)\right|$, then the \important{expectation} of $g\left(X, Y\right)$ is: 
...
\parag{Definition: Joint and central moments}{
    Let $X, Y$ be random variables of density or mass $f_{X, Y}\left(x, y\right)$ such that $\exval\left(\left|g\left(X,Y\right)\right|\right) < \infty$. Finally, let $r, s \in \mathbb{N}$.
...
\parag{Theorem: Expectation}{
    If $X$ and $Y$ are independent, and $g\left(X\right)$ and $h\left(Y\right)$ are functions which expetations exist, then we have: 
...
\parag{Theorem: Linearity}{
    ...
    \subparag{Remark}{
        Those properties can easily be fond back.
