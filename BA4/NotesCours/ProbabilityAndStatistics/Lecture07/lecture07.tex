...
\parag{Example}{
    Let us consider $Y = I\left(X \geq 1\right)$. We have: 
...
\parag{Definition: Expectation}{
    Let $X$ e a discrete random variable for which $\sum_{x \in D_X}^{} \left|x\right|f_X\left(x\right) < \infty$, where $D_X$ is the support of $f_X$. The \important{expectation} (or \important{expected value} or \important{mean}) of $X$ is: 
...
\parag{Theorem: Expected value of a function}{
    Let $X$ be a random variable with mass function $f$, and let $g$ be a real-values function of $X$. Then: 
