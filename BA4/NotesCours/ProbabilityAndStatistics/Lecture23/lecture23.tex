...
\parag{Remark}{
    The bias is important in order to choose the estimator, but it is not the only facto we have to take into account. The variance of the estimator is also really important (recall that the variance represents the mean squared distance to the expected value).
}
...
\parag{Definition: Pivot}{
    ...
    A \important{pivot} is a function $Q = q\left(Y; \theta\right)$ which is independent of $\theta$. We say that $Q$ is \important{pivotal}.
}
...
\parag{Construction of a CI}{
    To construct a CI, we first need find a pivot $Q = q\left(Y, \theta\right)$. We then need to choose $\alpha_L$ and $\alpha_U$, and obtain the quantiles $q_{\alpha_U}, q_{1 - \alpha_L}$ of $Q$. By definition of quantiles, this gives us: 
