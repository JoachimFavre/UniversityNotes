...
\parag{Proposition: CNF}{
    ...
    \subparag{Proof}{
        We can just construct the CNF of $\lnot f$, and then take its negation. Every $\land$ will be turned to an $\lor$, and inversely, thanks to Morgan's Laws. This indeed implies that we get a CNF.
    }
...
\parag{Lemma: SAT}{
    ...
    \subparag{Proof}{
        ...
        We notice that applying any operator exactly once leads a CNF and a DNF formula: $x_1 \lor \ldots \lor x_n$, $x_1 \land \ldots \land x_n$ and $\lnot x_1$ are all both CNFs and DNFs. Moreover, $a \leftrightarrow b$ where $a$ and $b$ are CNFs or DNFs can be converted into a CNF in constant time. Finally, and-ing CNF formulas also yield a CNF formula. This means that we managed to turn our circuit into a CNF formula in constant time.
