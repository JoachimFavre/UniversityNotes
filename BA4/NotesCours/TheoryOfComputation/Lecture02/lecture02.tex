...
\parag{Theorem: Complement}{
    ...
    \subparag{Remark}{
        We just swap the accepting state and the non-accepting state. This naturally means that languages that were accepted are no longer, and inversely.
    }
}
...
\parag{Definition: NFA}{
    ...
    \begin{itemize}
        ...
        \item $F \subseteq Q$ is the set of accept states, where we allow $F = \Omega$.
    \end{itemize}
...
\parag{Example 1}{
    ...
    \subparag{Observation}{
        Note that, to prove we wanted to show that an input is accepted, we don't need to draw the whole tree, only the branch going to an accepting state. This is named guess and verify.
    }
...
\parag{Example 3}{
    ...
    This machine is the union of two regular languages: $w \in \left\{0\right\}^*$ with a length 2, and $w \in \left\{0\right\}^*$ with 3 states. This construction is more efficient in size, and it gives: 
    \[L = \left\{w \in \left\{0, 1\right\}^* \suchthat w \text{ has length 2 or 3}\right\}\]
}
