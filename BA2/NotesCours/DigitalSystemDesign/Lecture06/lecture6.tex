...
\parag{Fixed point representation}{
    ...
    Doing it in the other direction, if we want to turn $17.3$ into base 5, we can do two steps. First, using algorithms we have already seen, we get $17 = 32_{B=5}$. To get the decimal part, we do multiplications. We see that $0.3\cdot 5 = 1.3$, so we have the $1$ after the coma, and then we continue our multiplications on the rest, $0.3$. If we get a 0, we are done (we will never get a 0 here, we would have a periodic decimal).
}
