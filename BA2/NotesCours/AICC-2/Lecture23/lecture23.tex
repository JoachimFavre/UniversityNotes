...
\parag{Theorem}{
    ...
    \subparag{Proof}{
        To proof this, we will first show that $\min w\left(\bvec{x}\right) \geq d_{min}\left(\mathcal{C}\right)$, and then $\min w\left(\bvec{x}\right) \leq d_{min}\left(\mathcal{C}\right)$.
...
\parag{Example 1}{
    ...
    We notice that the first three bits are exactly $\bvec{u}$, and then the following bits show the partis of the different bits of $\bvec{u}$ (for instance, the fourth bit is the parity of the first bit plus the second bit).
}
