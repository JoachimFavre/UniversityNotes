...
\begin{itemize}[left=0pt]
    ...
    \item Explanation of El Gammal's cryptosystem.
\end{itemize}
...
\parag{El Gammal's cryptosystem}{
    There is a ciphering system other than RSA that works with trapdoor one-way functions, named \important{El Gammal's crypto}.
    ...
    \subparag{Personal note: Difference with Diffie-Hellman}{
        Note that the difference between El Gammal's cryptosystem and Diffie-Hellman's one, is that the later one allows to exchange keys, whereas the former allows to send a message privately.
    }
}
