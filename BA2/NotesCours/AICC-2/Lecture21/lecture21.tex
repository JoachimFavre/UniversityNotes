...
\parag{Definition: Vector space}{
    $\left(\mathcal{V}, +, \cdot\right)$, where $\mathcal{V}$ is a non-empty set $\mathcal{V}$, is a \important{vector space} over a field $\mathbb{F}$ if the following properties hold:
    \begin{enumerate}
