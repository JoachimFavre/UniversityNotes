...
\parag{Example}{
    ...
    \subparag{Intuition}{
        If we knew that the first child is a girl, the probability that the second child is a girl is $\frac{1}{2}$. Note that their is a difference in the problem which just solve: we know that \textit{one of the children} is a girl.

        The Professor says that, probably, the human brain is maybe not so good at dealing with probabilities. 
    }
}
...
\parag{Probability distribution of a random variable}{
    ...
    We know how to computer such events, so we can compute $p_X\left(x\right)$:
