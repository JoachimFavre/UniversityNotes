...
\parag{Twenty questions}{
    ...
    \subparag{Strategy}{
        ...
        Let's suppose that the realisation is $X = b$ (but we do not it). The first question is: is $x = e$? Since the answer is no, we know the first bit is 0. Then, we can ask ``is $x \in \left\{c, d\right\}$?'' And so on.
    }
