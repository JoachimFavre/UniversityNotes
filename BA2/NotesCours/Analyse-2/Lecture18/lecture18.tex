...
\parag{Exemple}{
    ...
    \[\frac{\partial^2 f\left(x, y\right)}{\partial x \partial y}\left(0, 1\right) = \frac{\partial^2 f}{\partial y \partial x}\left(0, 1\right) = -4ye^{-x + 2y^2 + 1}\eval_{\left(0, 1\right)}^{} = -4e^3\]
...
\parag{Taylor en trois dimensions}{
    ...
    \subparag{Remarque personnelle}{
        Je vois un pattern avec la formule trouvée dans le cours précédent. Ainsi, j'en sais rien si c'est vrai, mais je conjecture que nous avons, pour $n$ variables:
...
\parag{Exemple}{
    ...
    Nous voulons calculer sa formule de Taylor d'ordre 2.
...
\parag{Proposition: Condition nécessaire pour un extremum local}{
    ...
    \subparag{Preuve}{
        ...
        Nous savons que $\nabla f\left(\bvec{a}\right)$ existe implique que $g_i\left(x\right)$ est dérivable en $a_i = x$. De plus, nous savons qu'elle admet un extremum local en ce point puisque $f$ en a un à ce point (si $g$ n'avais pas d'extremum local, alors clairement $f$ n'en n'aurait pas non plus). Or, par le cours d'Analyse 1, cela implique que:
