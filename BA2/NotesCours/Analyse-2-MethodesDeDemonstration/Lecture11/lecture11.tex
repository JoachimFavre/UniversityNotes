...
\parag{Définition: Suite de Fibonacci}{
...
\parag{Exemple}{
    ...
    \subparag{Preuve}{
        ...
        \item Passons maintenant à l'hérédité. Puisque nous utilisions le principe de la récurrence généralisée, nous supposons que $P\left(n\right)$ et $P\left(n + 1\right)$ sont vraies (HR), pour un $n \geq 3$. Nous voulons démontrer que $P\left(n+2\right)$ est vraie: 
            \begin{multiequality}
            3f_{n+2} \over{=}{déf}\ & 3f_n + 3f_{n+1} \\
            \over{=}{HR}\ & \left(f_{n + 2} + f_{n-2}\right) + \left(f_{n + 3} + f_{n-1}\right) \\
            =\ & \underbrace{f_{n+2} + f_{n+3}}_{= n} + \underbrace{f_{n-2} + f_{n-1}}_{= f_n} \\
            =\ & f_{n+4} + f_n 
            \end{multiequality}
