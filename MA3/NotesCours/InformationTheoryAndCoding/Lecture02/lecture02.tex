...
\begin{parag}{Definition: Entropy}
    ...
    where we write $p\left(u\right) = \prob\left(U = u\right)$ and where we define $0 \log_2\left(\frac{1}{0}\right) = 0$ by continuous extension.

    \begin{subparag}{Remark}
        Note that, for some function $f: \mathbb{R} \mapsto \mathbb{R}$, then $f\left(U\right)$ is typically a random variable. Now, $H\left(U\right) \in \mathbb{R}$ is a scalar, so this is a bit of a bad notation. This is completely similar to $\exval\left(U\right) \in \mathbb{R}$.
    \end{subparag}
\end{parag}
