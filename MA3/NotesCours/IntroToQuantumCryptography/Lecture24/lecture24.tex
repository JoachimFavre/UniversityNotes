...
\begin{parag}{Theorem}
    ...
    \begin{itemize}
    \item \textit{(Encryption)} We describe encryption $\Enc_k\left(\rho\right)$. Let $\rho$ be arbitrary. We sample some $r \leftarrow_U \left\{0, 1\right\}^{2n}$, evaluate $\left(u, v\right) \leftarrow F_k\left(r\right)$ for $u, v \in \left\{0, 1\right\}^n$, and output the following $4n$-qubit state:
