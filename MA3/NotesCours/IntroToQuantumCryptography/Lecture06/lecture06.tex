...
\begin{parag}{Bomb testing}
    ...
    \begin{subparag}{Intuition}
        The idea is that if it is a bomb, then it will measure the state and explode depending on the outcome; but if it is a dud, it will do nothing. We will try to exploit the measurement or its absence.
    \end{subparag}
    ...
    \begin{subparag}{Proof}
        ...
        \begin{itemize}
            \item Apply $\displaystyle R_{\delta} = \begin{pmatrix} \cos\left(\delta\right) & -\sin\left(\delta\right) \\ \sin\left(\delta\right) & \cos\left(\delta\right) \end{pmatrix}$ on the state.
