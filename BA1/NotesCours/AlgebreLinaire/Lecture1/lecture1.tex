...
\parag{Trouver toutes les solutions}{
    On a un système linéaire de $m \times n$ (avec $m$, le nombre d'équation, et $n$ le nombre de variables) avec coefficients $a_{ij}$ (avec $i \in \left\{1, \ldots, m\right\}$ et $j \in \left\{1, \ldots, m\right\}$):
    \begin{systemofequations}{}
    &\ a_{11} x_1 + \ldots + a_{1n} x_n = b_1 \\
    &\ \vdots \\
    &\ a_{m1} x_1 + \ldots + a_{mn} x_n = b_m
...
\parag{Pour aller plus loin}{
    ...
    \begin{enumerate}
        ...
        \item On peut \important{simplifier davantage} (en transformant la matrice en en ``diagonale'' et non en ``triangulaire'')
