...
\begin{parag}{Positive semi-definite operator}
    An operator $\hat{A}$ is \important{positive semi-definite} if it is Hermitian and all its eigenvalues are non-negative.
...
\begin{parag}{Partial trace}
    Let $\hat{A}$ be an operator acting on a Hilbert space $\mathcal{H}_{S \cup E} = \mathcal{H}_S \otimes \mathcal{H}_E$, and let $\ket{A_j} \otimes \ket{B_i}$ be a basis of this Hilbert space for $j \in \left\{1, \ldots, J\right\}$ and $i \in \left\{1, \ldots, I\right\}$. The $\left(k, \ell\right)$ component of the \important{partial trace} of $\hat{A}$ with respect to $\mathcal{H}_S$ is: 
...
\begin{parag}{Bloch ball}
    ...
    \begin{subparag}{Proof}
        ...
        \autoeq{0 \leq \det\left(\hat{\rho}\right) = \frac{1}{4} \det\begin{pmatrix} 1 + a_z & a_x - i a_y \\ a_x + i a_y & 1 - a_z \end{pmatrix}  = \frac{1}{4} \left(1 - a_x^2 - a_y^2 - a_z^2\right)= \frac{1}{4}\left(1 - \left\|\bvec{a}\right\|^2\right)}
