...
\begin{parag}{Definition: Polynomial ring}
    Let $x$ be a symbol and $S$ be some ring. We define the \important{polynomial ring} as: 
    \[S\left[x\right] = \left\{a_0 + a_1 x + \ldots + a_n x^n \suchthat n \in \mathbb{N}, a_0, \ldots, a_n \in S\right\}\]

    \begin{subparag}{Remark}
        We can also consider $x$ to be a number. For instance, picking $x = \sqrt{2}$, we have: 
        \autoeq[s]{S\left[\sqrt{2}\right] = \left\{a_0 + a_1 \sqrt{2} + a_2\cdot 2 + a_3\cdot 2\sqrt{2} + \ldots + a_n \sqrt{2}^n \suchthat n \in \mathbb{N}, a_0, \ldots, a_n \in S\right\} = \left\{a + b \sqrt{2} \suchthat a, b \in S\right\}}
    \end{subparag}
    
    \begin{subparag}{Observation}
        We notice that, for any ring $S$, then:
        \[0, 1 \in S\left[x\right]\]

        Indeed, we can just set $n= 0$ and $a_0 \in \left\{0, 1\right\}$. In fact, one can show that $S\left[x\right]$ is a ring as well.
    \end{subparag}
\end{parag}
...
\begin{parag}{Example 3}
    ...
    \begin{subparag}{Personal remark}
        By a completely similar reasoning, $\mathbb{Q}\left[\sqrt{5}\right]$ is also a field. Now, there exists a closed form formula for the $n$\Th Fibonacci number:
        \[F_n = \frac{\phi^n - \left(-\phi\right)^{-n}}{\sqrt{5}}\]
        where $\phi = \frac{1 + \sqrt{5}}{2} \in \mathbb{Q}\left[\sqrt{5}\right]$ is the golden ratio. Hence, using fast exponentiation on the ring $\mathbb{Z}\left[\sqrt{5}\right]$ (which elements can easily be represented on a computer, contrary to real numbers), we can compute large Fibonacci numbers very efficiently. This may give a quick motivation to study this \textit{field} (although algebra is great and this should be a motivation on its own \wink).

        This is presented in more details in the following video from Sheafification of G (more precisely at 20 minutes, for the golden medallist, although the context is likely important):
        \begin{center}
            \url{https://www.youtube.com/watch?v=KzT9I1d-LlQ}
        \end{center}
    \end{subparag}
\end{parag}
...
\begin{parag}{Example}
    ...
    As mentioned earlier, this is indeed a commutative ring: it is closed under addition and multiplication, which are commutative and associative, and we have additive inverses.
