...
\title{Introduction to Machine Learning --- BA3 \\ Summary}
...
\parag{Notations}{
    ...
    \[W = \begin{pmatrix} \bvec{w}_{\left(1\right)}^{T} \\ \vdots \\ \bvec{w}_{\left(C\right)}^{T} \end{pmatrix} \in \mathbb{R}^{\left(D+1\right)\times C}\]
}
...
\parag{Gradient descent}{
    ...
    \subparag{Stochastic gradient descent}{
...
\parag{Decision boundary}{
    ...
    Since it is the set of points which are equivalently distance to both labels, the equation of this boundary is given by: 
    \[\hat{\bvec{y}} = \frac{y_{pos} + y_{neg}}{2}\]
...
\parag{Ridge regression}{
    ...
    \[W^* = \left(\Phi^T \Phi + \lambda I_F\right)^{-1} \Phi^T \bvec{y}\]
    where $I_F$ is the $F \times F$ identity matrix.
    
    \subparag{Linear regression}{
        Leaving $\lambda = 0$, we get the special case of \important{linear regression}. Then, the closed-form formula can be rephrased as: 
        \[W^* = \left(\Phi^T \Phi\right)^{-1} \Phi^T \bvec{y} = \Phi^{\dagger} \bvec{y}\]
        where $\Phi^{\dagger}$ is the Moore-Penrose pseudo-inverse.
    }

    \subparag{Kernelisation}{
        Using the representer theorem, we can find that: 
        \[\bvec{\alpha}^* = K^{-1} \bvec{y}\]
        
        This is not of much use on its own, but we can use this result to find that: 
        \[\hat{\bvec{y}} = \bvec{y}^T K^{-1} k\left(X, \bvec{x}\right)\]
        
        Note that the value $\bvec{y}^T K^{-1}$ can be computed once during training, and the be reused at every evaluation of the model.
    }

    \subparag{Classification}{
...
\parag{Logistic regression}{
    ...
    \[\hat{y}^{\left(k\right)} = \frac{\exp\left(\bvec{w}^T_{\left(k\right)}\right)}{\sum_{j=1}^{C} \exp\left(\bvec{w}^T_{\left(j\right)} \bvec{x}\right)} \in \left[0, 1\right]\]
...
\parag{Support vector machine}{
    ...
    \subparag{Dual problem}{
        ...
        In other words, for every sample, either of those terms is equal to 0. The samples for which $\alpha_i^* \neq 0$ are the support vectors. However, most samples are not not support vectors, so we can decrease the computations by leaving $\mathcal{S}$ to be the set of support vectors: 
...
\paragsubparag{}{Primal derivation}{
    ...
    \[\hat{y}\left(\bvec{x}\right) = w^{\left(0\right)} + \widetilde{\bvec{w}}^T \bvec{x} = w^{\left(0\right)} + \widetilde{\bvec{w}}^T \bvec{x}_{\perp} + r \frac{\widetilde{\bvec{w}}^T \bvec{w}}{\left\|\widetilde{\bvec{w}}\right\|} = y\left(\bvec{x}_{\perp}\right) + r\left\|\widetilde{\bvec{w}}\right\|\]
...
\parag{$K$-nearest neighbours}{
    ...
    \subparag{Curse of dimensionality}{
...
\parag{Neural networks}{
    ...
    \subparag{Convolutional network}{
...
\parag{Principle component analysis}{
    ...
    \subparag{Kerenelisation}{
        PCA can be kernalised in a non-trivial fashion.

        First, we need to account for the fact that our data may not be centered in input-space, letting: 
