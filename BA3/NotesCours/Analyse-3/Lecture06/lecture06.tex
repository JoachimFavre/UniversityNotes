...
\parag{Exemple 2}{
    ...
    \[\begin{split}
    \gamma: \left[0, \pi\right] &\longmapsto \Gamma_2 \\
...
\section{Surfaces et intégrales de surfaces}
\subsection{Surfaces}
...
\parag{Passage entre les représentations}{
    ...
    \subparag{Cartésienne vers paramétrique}{
        Si nous connaissons: 
    ...
    \subparag{Paramétrique vers cartésien}{
        Si nous avons: 
...
\parag{Exemple}{
    ...
    \left(\theta, \phi\right) &\longmapsto \begin{pmatrix} R\sin\left(\phi\right)\cos\left(\theta\right) \\ R\sin\left(\theta\right)\left(\phi\right) \\ R\cos\left(\phi\right) \end{pmatrix} 
    \end{split}\]
...
\parag{Définition: Intégrale de surface}{
    ...
    \subparag{Remarque 2}{
        L'intégrale d'un champ scalaire $\iint_{\Sigma} f ds$ est indépendante de $\sigma$. Aussi, l'intégrale de $\iint_{\Sigma} F \dotprod ds$ est indépendante de $\sigma$, au signe près (cela dépend de l'orientation de $\Sigma$, que nous définirons juste \later{?} après).
    }
}
