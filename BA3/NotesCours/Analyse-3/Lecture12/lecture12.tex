...
\parag{Définition: Produit de convolution}{
    ...
    L'idée est que nous gardons la fonction $g$ fixée, que nous retournons (prenons la symétrie de) la fonction $f$ et la déplaçons pour que sont centre soit en $x$, puis calculons l'intégrale de leur produit.
...
\parag{Exemple 1}{
    ...
    \[f * g \left(x\right) = \int_{-\infty}^{\infty} f\left(x - t\right)g\left(x\right) dx = \int_{-1}^{1} f\left(x - t\right)dt = -\int_{x+1}^{x-1} f\left(y\right)dy = \int_{x-1}^{x+1} f\left(y\right)dy\]
...
\parag{Application 1: Résolution d'EDO}{
...
\parag{Application 2: Résolution d'EDO}{
...
\parag{Application 3: Résolution d'équations fonctionnelles}{
