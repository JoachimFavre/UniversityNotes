...
\parag{Application: Calcul de série}{
...
\parag{Remarque}{
    ...
    Si $f$ est paire, alors $f\left(x\right)\sin\left(\frac{2\pi}{T} nx\right)$ est impaire et $T$-périodique. Ainsi, puisque les $b_n$ sont calculs en intégrant cette fonction entre $-\frac{T}{2}$ et $\frac{T}{2}$, alors on trouve que, pour tout $n$: 
    \[b_n = 0\]
...
\parag{Définition: Série de Fourier en sinus}{
    Soit $f \in C_{morc}^1\left(\left[0, L\right]\right)$.
