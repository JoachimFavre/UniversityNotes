...
\subsection{Conditioning of linear systems}
 \parag{Vector norm}{
    ...
    \[\left\|\bvec{x}\right\|_p = \left(\sum_{i=1}^{n} \left|\bvec{x_i}\right|^p\right)^{\frac{1}{p}}\]
...
\parag{Conditioning number}{
    ...
    \subparag{Geometric interpretation}{
        We can internet geometrically the conditioning number:
...
\parag{Finding a parabola}{
    ...
    \subparag{Conditioning of the Vandermonde matrix}{
...
\parag{Regression}{
    ...
    \autoeq{\unexpanded{\left\|A \bvec{x} - \bvec{b}\right\|^2 = \left\langle A \bvec{x} - \bvec{b}, A \bvec{x} - \bvec{b}\right\rangle = \left\langle A \bvec{x}, A \bvec{x}\right\rangle - 2\left\langle A \bvec{x}, \bvec{b}\right\rangle + \left\langle\bvec{b}, \bvec{b}\right\rangle = \bvec{x}^T A^T A \bvec{x} - 2 \bvec{b}^T A \bvec{x} + \left\|\bvec{b}\right\|^2}}
    ...
    \subparag{Problem}{
        ...
        In fact, the QR decomposition allows us to solve this problem.
    }
}
