...
\parag{Tikhonov regularisation}{
    ...
    \autoeq{\unexpanded{\bvec{x}_{sol} = \argmin \left(\bvec{x}^T A^T A \bvec{x} - 2\bvec{b}^T A \bvec{x} + \left\|\bvec{b}\right\|_2^2 + \lambda \left\|\bvec{x}\right\|_2^2\right) = \argmin\left(\bvec{x}^T \left(A^T A + \lambda I \right) \bvec{x} - 2 \bvec{b}^T A \bvec{x} + \left\|\bvec{b}\right\|_2^2\right)}}
