...
\parag{Differentiation}{
    ...
    where $J_{x}^f$ means the Jacobian of the function $f$ with respect of the parameter $x$.
    ...
    Note that this special case of automatic differentiation applied to neural networks is called backpropagation.
}
...
\parag{Convolution NNs}{
    ...
    In a convolutional neural networks, we do multiple convultions in between multiple layers, and we also allow the neural network to also optimise those convolutions.
}
