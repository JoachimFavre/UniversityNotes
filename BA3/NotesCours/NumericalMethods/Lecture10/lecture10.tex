...
\parag{RBF interpolation}{
    ...
    The idea of \important{Radial Basis Function (RBF) interpolation}, is that, given $n$ evaluations $f\left(\bvec{x_i}\right)$ at positions $\bvec{x_i}$, we can just use a weighted average using also the distance to every point: 
    \[g\left(\bvec{x}\right) = \sum_{i=0}^{n} w_i \phi\left(\left\|\bvec{x} - \bvec{x_i}\right\|\right) f\left(\bvec{x_i}\right)\]
    ...
    Finally, we need to find the $w_i$ such that $g\left(\bvec{x_i}\right) = f\left(\bvec{x_i}\right)$ for all $i$ (those parameters are only here because, when doing interpolation, this property is quite important). This is just a linear system.
...
\parag{Remez Algorithm}{
    ...
    The idea of the algorithm is to first make an approximation (for instance using Chebyshev nodes). Then, we search for the zeroes of our function through bisection search, and use those information to make a golden bisection search to find the minimum and maximum of our approximation. After that, we make a new approximation by forcing those points to be at distance $E$ of our function real value. We don't know $E$, so we can put it as an unknown: 
    ...
    where $f$ is the function we want to approximate, $x_0, \ldots, x_n$ were the minimums and maximums of our approximation, and $a_0, \ldots, a_n$
    
    Now, the new approximation we get does not necessarily have its maximum and minimum at the same distance from the true function value. However, every iteration gets better. In other words, as we run this algorithm, we will converge to the following kind of result:
...
\parag{Summary}{
    ...
    \begin{enumerate}
        \item We choose a good interval of approximation: the smaller the better. 
