...
\parag{Definition: Average throughput}{
    The \important{average throughput} is the average rate at which the destination receives data.

    \subparag{Example}{
        If the destination receives $\SI{1}{\giga\byte }$ of data in $\SI{1}{\minute }$. Then, the average throughput is $\SI{133.34}{\mega\byte \per  \second }$.
    }
}
