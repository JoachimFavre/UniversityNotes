...
\parag{Max-flow min-cut theorem}{
    Let $G = \left(V, E\right)$ be a flow network, with source $c$, sink $t$, capacities $c$ and flow $f$. Then, the following propositions are equivalent:
    \begin{enumerate}
...
if no augmenting path exists in the residual network:
    ...
    set T = V \ S
// (S, T) is a maximum cut
\end{filecontents*}


\parag{Summary}{
    All this shows that our Ford-Mulkerson method for finding a max-flow works:
