...
\parag{Breadth-First Search}{
    ...
    \subparag{Observation}{
        We note that BFS may not reach all the vertices.
    }
...
\parag{Depth-First Search}{
    BFS goes through every vertex, but not necessarily every edge. We would now want to make an algorithm that goes through every edges. Note that this algorithm may seem very abstract and \textit{useless} for now but, as we will see right after, it gives us a very interesting insight about a graph.
