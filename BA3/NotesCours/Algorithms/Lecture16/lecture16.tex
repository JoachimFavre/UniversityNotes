...
\lecture{16}{2022-11-18}{Shipping cheese through pipes}{
...
\parag{Definition: Augmenting path}{
   ...
   \important{Augmenting the flow} $f$ by this path means applying the minimum capacity over the path: add it to edges which were here at the start, and remove it to edges we added after through the residual capacity.
}
...
\parag{Definition: Net flow across a cut}{
    ...
    \[f\left(S, T\right) = \sum_{u \in S, v \in T}^{} f\left(u, v\right) - \sum_{u \in S, v \in T}^{} f\left(v, u\right)\]
    ...
    \subparag{Observation}{
        We notice that this value is equal to the flow on the network $\left|f\right|$, by conservation of flow. This means that the net flow across a cut does not depend on the cut.
    }
}

\parag{Definition: Capacity of a cut}{
    The \important{capacity of a cut} $S, T$ is defined as: 
    \[c\left(S, T\right) = \sum_{u \in S, v \in T}^{} c\left(u, v\right)\]
