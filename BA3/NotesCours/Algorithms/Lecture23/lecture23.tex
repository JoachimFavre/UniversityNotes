...
\parag{Proposition: Randomised quick sort expected running time}{
    ...
    \subparag{Proof}{
        ...
        Let's call $z_1, \ldots, z_n$ the elements of $A$, in a way such that $z_1 \leq \ldots \leq z_n$. Also, let $Z_{ij} = \left\{z_i, z_{i+1}, \ldots, i_j\right\}$, $C_{ij}$ be the event that $z_i$ is compared to $z_j$ throughout the execution of the algorithm, and $X_{ij} = I\left(C_{ij}\right)$. 

        As each pair is compared at most once (when of them is the pivot), the total number of comparisons formed by the algorithm is: 
